...
面向团队协作的工作记录与任务管理平台，支持任务看板、计划提醒、评论协作、数据统计及 AI 周报/年度报告生成。
...
![桌面客户端绑定 API Key](docs/images/client_login.png)

**桌面客户端 · 工作台**

![桌面客户端工作台](docs/images/client_index.png)
...
- **开始/截止日期联动**：新建与编辑弹窗中，把开始日期改到晚于截止日期时截止日期自动顺延，把截止日期改到早于开始日期时开始日期自动提前，避免出现"开始晚于截止"的区间；打开弹窗回填历史数据时不触发该联动，不会在用户未操作时改写既有日期。桌面客户端同规则
...
- **桌面快捷客户端**：macOS 优先的 Tauri 菜单栏应用，窗口内 `⌘N` 新建任务、`⌘K` 搜索，唤起方式交给 Manico 等第三方启动器；支持分配给我 / 我创建的 / 团队可见视图、任务搜索筛选、逾期置顶分组、状态切换（待办 / 进行中 / 已完成 / 已取消）、通知中心（含全部已读、点击直达任务）、快速新建（可选「待办」不排期 + 纯文本详细内容）、开机启动与打开网页版。客户端「详细内容」为纯文本，需要插图或附件时在网页版补充
- **个人 API Key**：用户可在「个人设置 → 桌面客户端与 API Key」创建多个命名密钥并随时吊销；完整密钥只展示一次，服务端仅存 SHA-256 摘要，桌面端加密存放在本机应用目录
...
| 桌面端 `desktop/` | Tauri 2 + Rust + Vue3 + TypeScript（系统托盘、单实例、本机加密凭据） |
| 共享包 `packages/shared/` | TypeScript 类型 + 可配置 Axios API 客户端 |
...
前置：Node 20+、Rust stable，以及已经运行的 work-report 服务端。
...
# 单元测试、TypeScript 构建、Rust fmt/clippy/test
make check-desktop

# 生成 macOS .app / .dmg（未签名的开发构建）
make build-desktop
```

构建产物位于 `desktop/src-tauri/target/release/bundle/`。

首次启动时填写平台外网地址和个人 API Key。客户端会通过 `/api/auth/me` 验证身份；平台地址写入普通应用配置，API Key 用 ChaCha20-Poly1305 加密后写入 `~/Library/Application Support/com.daizl.workreport/credentials.bin`（目录 0700、文件 0600）。密钥由机器指纹（`IOPlatformUUID`）与本机随机盐派生，凭据文件复制到其他电脑无法解密；实际保护强度取决于文件权限与全盘加密，因此不会像钥匙串那样弹出系统密码框。正式分发前需配置 Apple Developer 签名和公证，证书与密码不得提交仓库。

客户端不注册全局快捷键，唤起方式交给 Manico、Raycast 等第三方启动器。窗口激活时可用：

- `⌘N`：新建任务
- `⌘K`：聚焦任务搜索
- `J` / `K` 或方向键：浏览任务
- `⌘Enter`：提交新任务或切换当前任务完成状态
- `Esc`：关闭当前面板，再次按下隐藏窗口

菜单栏图标左键单击提供「显示主界面」和「退出」两项。左上角关闭按钮只收起界面，客户端继续在菜单栏驻留，再次点击 Dock 图标或菜单栏「显示主界面」即可唤回。
...
GET/POST /api/api-keys         列出 / 创建本人 API Key（仅网页登录会话）
DELETE /api/api-keys/:id       吊销本人 API Key（仅网页登录会话）
GET  /api/users                用户列表
...
- **UserAPIKey**：用户长期访问凭据（名称 / 展示前缀 / SHA-256 摘要 / 可选过期时间 / 最后使用时间）；完整密钥不入库且只在创建时返回一次
